...
def _write_csv(path: Path, arr: np.ndarray) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["ch1", "ch2", "ch3", "ch4", "ch5", "ch6", "ch7", "ch8"])
        for row in arr:
...
def _make_signal(length: int, amp: float) -> np.ndarray:
    ...
    sig = np.stack([amp * np.sin(t + i) for i in range(8)], axis=1)
...
def test_quality_filter_removes_low_energy_windows(tmp_path: Path):
    ...
    weak = np.zeros((420, 8), dtype=np.float32)
...
def test_multi_phase_expands_samples(tmp_path: Path):
    ...
    assert samples.shape[1:] == (16, 24, 6)
...
def test_loader_rejects_legacy_six_channel_csv(tmp_path: Path):
    gestures = {"RELAX": 0}
    cfg = PreprocessConfig()
    sig = np.zeros((420, 6), dtype=np.float32)
    path = tmp_path / "RELAX" / "legacy.csv"
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["ch1", "ch2", "ch3", "ch4", "ch5", "ch6"])
        writer.writerows(sig.tolist())

    loader = CSVDatasetLoader(tmp_path, gestures, cfg, quality_filter=QualityFilterConfig(enabled=False))

    try:
        loader.load_all_with_sources()
    except RuntimeError as exc:
        assert "No samples loaded from dataset" in str(exc)
        assert "Unsupported CSV headers for 8-channel protocol" in str(exc)
    else:
        raise AssertionError("Expected legacy 6-channel CSV to be rejected")
